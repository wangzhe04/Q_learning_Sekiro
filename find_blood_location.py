...
def self_blood_count(self_gray):
    self_blood = 0
    for self_bd_num in self_gray[464]:
...
def self_body_dount(self_gray):
    self_body = 0
    for self_bd_num in self_gray[464]:
        # self blood gray pixel 80~98
        # 血量灰度值80~98
        print(self_bd_num)
        if self_bd_num > 30 and self_bd_num < 60:
            self_body += 1
        # print(self_blood)
    return self_body
...
def boss_blood_count(boss_gray):
    ...
    for boss_bd_num in boss_gray[0]:
        ...
        # 血量灰度值65~75
        # print(boss_bd_num)
        if boss_bd_num > 40 and boss_bd_num < 75:
            boss_blood += 1
    return boss_blood
...
blood_window = (320, 80, 720, 580) # (60, 95, 290, 572)
